...
def size_container_folding(value):
    ...
    if len(value) < MAX_LEN:
        if isinstance(value, list):
            return ast.List(list(map(to_ast, value)), ast.Load())
        elif isinstance(value, tuple):
            return ast.Tuple(list(map(to_ast, value)), ast.Load())
        elif isinstance(value, set):
            return ast.Set(list(map(to_ast, value)))
        elif isinstance(value, dict):
            keys = list(map(to_ast, iter(value.keys())))
            values = list(map(to_ast, iter(value.values())))
